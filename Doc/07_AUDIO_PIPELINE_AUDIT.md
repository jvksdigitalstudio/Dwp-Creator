...
> **⚠️ CORRECCIÓN POSTERIOR (ver `Doc/28_PHASE2_1_FLOAT32_DWP_AUDIT.md`, FASE 2.1):** varias frases de este documento (ej. "instrumento de 48 muestras en 32-bit float estéreo") describen el fixture real como float32 con el mismo nivel de confianza que el resto de hechos `[CODE]` de este documento. Esa clasificación depende de un `.wav` real examinado en una fase anterior del proyecto y ya no disponible en este repositorio — el propio `0x01F7` del `.dwp` (los 4 bytes/muestra que aquí se citan) no distingue por sí solo PCM entero de 32 bits de IEEE float de 32 bits. Reclasificado como 🟡 PARTIALLY CONFIRMED en `Doc/28`, no como un error de esta auditoría.
