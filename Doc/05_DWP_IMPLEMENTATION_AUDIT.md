...
> **⚠️ CORRECCIÓN POSTERIOR (ver `Doc/28_PHASE2_1_FLOAT32_DWP_AUDIT.md`, FASE 2.1):** la sección "Bloque `0x01f7`" de más abajo describe el sample 0 como "32-bit float" y presenta esa clasificación junto al resto de evidencia `[BINARY]` con el mismo nivel de confianza. Eso reflejaba correctamente lo que el `.wav` real disponible en ese momento (ya no presente en este repositorio) mostraba — pero **el campo `0x01F7` del `.dwp`, por sí solo, no distingue PCM entero de 32 bits de IEEE float de 32 bits** (ambos ocupan igual 4 bytes/32 bits): la clasificación "float32" depende enteramente de esa evidencia externa hoy no re-verificable, no de algo que el `.dwp` en sí codifique. `Doc/28` reclasifica esto como 🟡 PARTIALLY CONFIRMED, no como un error de esta auditoría.
