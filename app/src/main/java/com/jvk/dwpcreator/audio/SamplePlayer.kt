...
class SamplePlayer {
    ...
    companion object {
        /**
         * Hard safety cap on concurrent voices, not a "voice stealing"
         * engine (explicitly out of scope per Sección 17 del prompt
         * maestro). Each [play] call allocates its own native [AudioTrack];
         * without any limit, a fast MIDI note stream or rapid repeated
         * previews could keep piling up tracks until the platform starts
         * refusing to allocate more (a real, limited system resource on
         * Android). Simply declining a new voice past this cap is the
         * simplest safe behavior -- it never corrupts state, it just skips
         * sound for the excess voice.
         */
    ...
    fun play(wavBytes: ByteArray, velocity: Int = 127) {
        ...
        // Only mono/stereo output is configured below; any other channel
        // count (e.g. a stray 5.1 file) would otherwise silently get
        // mis-treated as stereo, playing back garbled audio with no error.
        if (decoded.channelCount !in 1..2) return
        ...
        synchronized(activeTracks) {
            if (activeTracks.size >= MAX_CONCURRENT_VOICES) {
                track.release()
                return
            }
