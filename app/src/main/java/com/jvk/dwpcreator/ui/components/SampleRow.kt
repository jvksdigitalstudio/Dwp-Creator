...
fun SampleRow(
    ...
    Row(
        ...
        // NOTE (audit finding BUG-01, Doc/12_KNOWN_ISSUES.md): this menu used
        // to also offer "Renombrar"/"Eliminar" per sample, but MainActivity
        // never wired those callbacks to a real DwpEngine operation -- they
        // were silent no-ops. Removed rather than implemented in this pass,
        // because a real per-sample rename/delete needs to also re-index
        // LoadedProject.audioByIndex (position-based today), which is a
        // larger, separate change. Only "Preview" is real, so only "Preview"
        // is offered.
        SampleContextMenu(
            expanded = menuOpen,
