...
object ZipProjectLoader {
    ...
    fun load(zipBytes: ByteArray): LoadedProject = load(ByteArrayInputStream(zipBytes))

    /**
     * Igual que [load] con un `ByteArray`, pero leyendo el zip directamente de
     * [input] a medida que se descomprime. Es la vía que usa la app: evita
     * materializar primero el `.zip` COMPLETO en memoria (cientos de MB para
     * un instrumento real) y después, además, todo su contenido descomprimido
     * -- el pico de memoria de una carga pasaba de "zip + contenido" a
     * "contenido". Cierra [input] al terminar.
     */
    fun load(input: InputStream): LoadedProject =
        load(input, MAX_ENTRIES, MAX_SINGLE_ENTRY_BYTES, MAX_TOTAL_UNCOMPRESSED_BYTES)
    ...
    ): LoadedProject = load(ByteArrayInputStream(zipBytes), maxEntries, maxSingleEntryBytes, maxTotalUncompressedBytes)

    /** Implementación real de todas las variantes de [load]; ver la de `ByteArray` para el porqué de los parámetros. */
    internal fun load(
        input: InputStream,
        maxEntries: Int,
        maxSingleEntryBytes: Long,
